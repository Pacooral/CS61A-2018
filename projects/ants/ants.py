...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while not cur_place.is_hive():
            if(self.min_range<=rage<=self.max_range) and (cur_place.bees!=[]) :
                return bee_selector(cur_place.bees)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if (self.chewing == 0) & (self.place.bees!=[]):
            self.eat(bee_selector(self.place.bees))
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        if (self.contained_ant==None) and (not hasattr(other,'contained_ant')):
            return True
        else :
            return False
        # END Problem 8
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.imposter==False:
            ...
            while cur_place!=None :
                if cur_place.ant!=None :
                    cur_place.ant.buff()
                    if hasattr(cur_place.ant,"contained_ant") and cur_place.ant.contained_ant!=None:
                        cur_place.ant.contained_ant.buff()
